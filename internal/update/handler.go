...
// Handler 把 Updater 暴露为 /api/admin 下的 HTTP 接口，持有更新路由所需依赖
type Handler struct {
	DB  *gorm.DB
	Cfg *config.Config
	up  *Updater
...
// Register 挂载管理员更新路由，全部经过 auth.Middleware 与 requireAdmin 双重守卫，仅管理员可访问
...
// trigger 使用 Manager 已校验候选发起更新，交接调用成功返回后由 Service 异步请求关闭
