...
func init() {
	registerEntries(map[string][2]string{
		...
		"account.capacity_zero_note":        {"留空时使用服务端默认容量策略。", "Leave empty for the server default."},
		"account.capacity_limit":            {"管理员上限：%d MiB", "Administrator limit: %d MiB"},
		"account.capacity_unlimited":        {"管理员未设置容量上限。", "No administrator capacity limit is configured."},
