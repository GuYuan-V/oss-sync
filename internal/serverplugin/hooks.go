...
// RunHook invokes every dynamically registered action or filter with the given name.
...
// ApplyHook keeps the existing blog host contract while using dynamic registrations.
...
// RenderAdminPage invokes the callback registered for one plugin admin page.
